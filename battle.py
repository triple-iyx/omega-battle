...
async def stats(ctx):
	...
	embed = discord.Embed(description='**HP**: {}/{}\n**Gold**: {}\n**XP**: {}\n**Weapon Equipped**: {}\n	**Weapon Damage**: {}'.format(hp, maxHP, gold, xp, weapon_e, weapon_d), color=discord.Colour.purple())
...
async def on_message(message):
	modmail = discord.Object(id='470032011726422016')
	with open('modmail.json', 'r') as f:
		mod = json.load(f)
	if message.author == client.user:
		return
	if message.channel.is_private:
		if not message.author.id in mod:
			mod[message.author.id] = {}
			mod[message.author.id]['name'] = message.author.name
			await client.send_message(message.channel, '**Please wait for a moderator to reply**.')
		r = await client.send_message(modmail, '**{} ({})**: {}'.format(message.author, message.author.id, message.content))
		with open('modmail.json', 'w') as f:
			json.dump(mod, f)
			
#reply
@client.command(pass_context=True)
async def r(ctx, user: discord.Member, *, msg):
	require = [role.name for role in ctx.message.author.roles]
	with open('modmail.json', 'r') as f:
		mod = json.load(f)
	if "Admin" in require:
		if user.id in mod:
			await client.add_reaction(ctx.message, emoji='📮')
			await client.wait_for_reaction('📮', user=ctx.message.author)
			await client.send_message(user, "**{}**: {}".format(ctx.message.author, msg))
		
#anonymous
@client.command(pass_context=True)
async def a(ctx, user: discord.Member, *, msg):
	require = [role.name for role in ctx.message.author.roles]
	with open('modmail.json', 'r') as f:
		mod = json.load(f)
	if user.id in mod:
		await client.add_reaction(ctx.message, emoji='📮')
		await client.wait_for_reaction('📮', user=ctx.message.author)
		await client.send_message(user, "**Server Mod**: {}".format(msg))
		
#close thread
@client.command(pass_context=True, name="close")
async def _close(ctx, user: discord.Member):
	require = [role.name for role in ctx.message.author.roles]
	with open('modmail.json', 'r') as f:
		mod = json.load(f)
	if user.id in mod:
		channel = discord.Object(id='470032011726422016')
		del mod[user.id]
		await client.send_message(user, '**This DM thread has been closed. Thank you for your query and/or assistance.**')
		await client.send_message(channel, '**({}) has been closed.**'.format(user.id))
		with open('modmail.json', 'w') as f:
			json.dump(mod, f)
#earn gold
@client.listen()
